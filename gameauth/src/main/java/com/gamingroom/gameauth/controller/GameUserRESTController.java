...
// 20220920 - define URL path annotation for gameusers
@Path("/gameusers/")
...
public class GameUserRESTController {
    ...
    // 20220920 - User must be authenticated. Allow API access to user with USER role
    @RolesAllowed({"USER"})
    ...
    // 20220920 -  User must be authenticated. Allow API access to user with ADMIN role
    @RolesAllowed({"ADMIN"})
